...
public abstract class ModdedTorchItem : ModItem
{
	...
	public abstract int Value { get; }
		...
		public override void SetDefaults() 
		{
			...
			Item.DefaultToTorch(TorchTileType, 0, false);
			Item.rare = Rarity;
			Item.value = Value;
		}
		...
		public override void HoldItem(Player player) 
		{
			...
			if (player.wet && !CanFunctionInWater) 
			{
				return;
			}

			if (player.lavaWet && !CanFunctionInLava)
			{
				return;
			}
			...
			{
				Dust dust = Dust.NewDustDirect(new Vector2(player.itemLocation.X + (player.direction == -1 ? -16f : 6f), player.itemLocation.Y - 14f * player.gravDir), 4, 4, SparkleDustType, 0f, 0f, 100);
