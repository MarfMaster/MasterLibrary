...
public abstract class ModdedCampfireTile : ModTile
{
    ...
    public void ToggleTile(int i, int j)
    {
        ...
        for (var y = topY; y < topY + 2; y++)
        {
            ...
            if (Wiring.running) Wiring.SkipWire(x, y);
        }

        if (Main.netMode != NetmodeID.SinglePlayer) NetMessage.SendTileSquare(-1, topX, topY, 3, 2);
    }
    ...
    public override void ModifyLight(int i, int j, ref float r, ref float g, ref float b)
    {
        ...
        if (tile.TileFrameY < 36)
        {
            ...
            r = LightColor.R + pulse;
            g = LightColor.G + pulse;
            b = LightColor.B + pulse;
        }
    }
    ...
    public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
    {
        ...
        if (tile.TileFrameY < 36)
        {
            var color = new Color(255, 255, 255, 0); //to draw at full brightness
            ...
            spriteBatch.Draw(flameTexture.Value,
                new Vector2(i * 16 - (int)Main.screenPosition.X, j * 16 - (int)Main.screenPosition.Y + offsetY) + zero,
